...
export class DocumentService {
	...
	updateDataFields(document: Document, fields): Observable<Document> {
		...
		return this.http.post(this.api + 'datafields/' + document.id, JSON.stringify(fields), {headers}).map(response => response.json()).catch(error => this.handleError(error, this.router));
	}

	createDocument(document: Document, templateId: number): Observable<Document> {
		let headers = new Headers();
  		headers.append('Content-Type', 'application/json');
		return this.http.post(this.api + 'documents/' + templateId, JSON.stringify(document), {headers}).map(response => response.json()).catch(error => this.handleError(error, this.router));
	}

	updateDocument(document: Document): Observable<Document> {
		let headers = new Headers();
  		headers.append('Content-Type', 'application/json');
		return this.http.put(this.api + 'documents/' + document.id, JSON.stringify(document), {headers}).map(response => response.json()).catch(error => this.handleError(error, this.router));
	}
